...
''';
  ...
  group('engraved bar numbers stripped', () {
    // Verovio writes the number inside the measure it belongs to, as a <g>
    // named after the MEI element. Note the FIRST one must go too — unlike
    // clef/keySig, where occurrence 0 is the one worth keeping.
    String measure(int n, String number) => '<g id="m$n" class="measure">'
        '$number'
        '<g class="staff"><g class="layer"><g class="note">n$n</g></g></g>'
        '</g>';
    String mNum(int n) =>
        '<g id="mn$n" class="mNum"><text x="0" y="0"><tspan>$n</tspan></text></g>';

    test('removes the number from every measure, first included', () {
      final svg = '<svg viewBox="0 0 100 100">'
          '${measure(1, mNum(1))}${measure(2, mNum(2))}${measure(3, mNum(3))}'
          '</svg>';
      final out = VerovioEngraver.stripMeasureNumbers(svg);
      expect(out, isNot(contains('mNum')));
      expect(out, isNot(contains('<tspan>')));
      // ...and leaves the music alone.
      for (final n in [1, 2, 3]) {
        expect(out, contains('<g class="note">n$n</g>'));
      }
      expect(RegExp(r'class="measure"').allMatches(out).length, 3);
    });

    test('an autogenerated number is removed — the two-token class', () {
      // The real-world case, read off `dev-iphone`: Verovio marks a number it
      // invented itself with a second class, and an exact `class="mNum"` compare
      // misses every one of them.
      final svg = '<svg viewBox="0 0 100 100">'
          '${measure(1, '<g id="mn1" class="mNum autogenerated">'
              '<text x="0" y="0"><tspan>1</tspan></text></g>')}'
          '${measure(2, mNum(2))}'
          '</svg>';
      final out = VerovioEngraver.stripMeasureNumbers(svg);
      expect(out, isNot(contains('mNum')));
      expect(out, isNot(contains('autogenerated')));
      expect(out, contains('<g class="note">n1</g>'));
      expect(out, contains('<g class="note">n2</g>'));
    });

    test('a class on a non-<g> element cannot take its group with it', () {
      // `lastIndexOf('<g')` from a `<path class="…">` lands on the enclosing
      // group; without the opening-tag guard, asking for that class would delete
      // the whole staff.
      const svg = '<svg viewBox="0 0 100 100">'
          '<g class="staff"><path class="mNum" d="M0 0" />'
          '<g class="layer"><g class="note">keep</g></g></g>'
          '</svg>';
      final out = VerovioEngraver.stripMeasureNumbers(svg);
      expect(out, contains('<g class="note">keep</g>'));
      expect(out, contains('class="staff"'));
    });

    test('the older measureNum spelling is removed too', () {
      final svg = '<svg viewBox="0 0 100 100">'
          '${measure(1, '<g class="measureNum"><text>1</text></g>')}'
          '</svg>';
      final out = VerovioEngraver.stripMeasureNumbers(svg);
      expect(out, isNot(contains('measureNum')));
      expect(out, contains('<g class="note">n1</g>'));
    });

    test('a score with no numbers is passed through unharmed', () {
      final svg = '<svg viewBox="0 0 100 100">${measure(1, '')}</svg>';
      expect(VerovioEngraver.stripMeasureNumbers(svg), svg);
    });

    test('a self-closing number group does not swallow the notes', () {
      // Same shape as the empty-keySig regression below: Verovio collapses an
      // empty node, and scanning on for a `</g>` would eat the next sibling.
      final svg = '<svg viewBox="0 0 100 100">'
          '${measure(1, '<g id="mn1" class="mNum" />')}'
          '</svg>';
      final out = VerovioEngraver.stripMeasureNumbers(svg);
      expect(out, isNot(contains('mNum')));
      expect(out, contains('<g class="note">n1</g>'));
